...
import BookNow from "../components/actions/BookNow";
    ...
    <div className="p-6 w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredTrips.length > 0 ? (
          <Suspense fallback={<p className="col-span-full text-center text-gray-500">Loading trips...</p>}>
            {filteredTrips.map((trip) => (
              <TripCard key={trip.id} trip={trip} people={people} onBook={handleBookClick} />
            ))}
          </Suspense>
        ) : (
          <p className="col-span-full text-center text-gray-500">No trips found matching your filters.</p>
        )}
      </div>
